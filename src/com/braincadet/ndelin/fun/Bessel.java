 ...
 * code ported from http://www.astro.rug.nl/~gipsy/sub/bessel.c

 Function:     BESSEL
 Purpose:      Evaluate Bessel function J, Yk, I, K of integer order.
 Category:     MATH
 File:         bessel.c
 ...
 has two solutions called Bessel functions of the first kind Jv(x) and Bessel functions of the second kind Yv(x).
 The routines bessj and bessy return the J and Yk for integer v and therefore are called Bessel functions of integer order.
 ...
 has two solutions called modified Bessel functions Iv(x) and Kv(x).
 The routines bessi and bessk return the I and K for integer v and therefore are called Modified Bessel functions of integer order.
 (Abramowitz & Stegun, Handbook of mathematical functions, ch. 9, pages 358,- and 374,- )

 The implementation is based on the ideas from Numerical Recipes, Press et. al.

 double   result;
 result = bessj( int n, double x )
 result = bessy( int n, double x )
 result = bessi( int n, double x )
 result = bessk( int n, double x )
...
public class Bessel {
    ...
    public static final double ACC = 40.0f;
